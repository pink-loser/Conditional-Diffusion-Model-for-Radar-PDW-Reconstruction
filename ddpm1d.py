...
class Unet1D(Module):
    ...
    def forward(self, x, time, x_self_cond = None, self_cond = None):
        x_self_cond = default(x_self_cond, self_cond)
        ...
        # 原始输出：模型预测的残差部分
        pred_residual = self.final_conv(x)
        # Method 3: 显式锚点连接 (Explicit Anchor Connection)
        # 如果存在 self_condition (即传入的受损数据)，将其前 4 个通道作为锚点加回
        if self.self_condition and x_self_cond is not None:
            # 假设 pred_residual 输出 4 或 8 通道，取前 4 位
            anchor = x_self_cond[:, :self.out_dim, :]
            return pred_residual + anchor
...
class Transformer1D(Module):
    def __init__(
        ...
        # 1. 初始投影：将通道维度映射到 Transformer 的 Embedding 维度
        self.init_proj = nn.Conv1d(input_channels, dim, 1)

        # 2. 位置编码
        self.pos_emb = nn.Parameter(torch.randn(1, 4096, dim)) # 针对你的 SEQ_LEN=4096
        ...
        self.time_mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(time_dim, dim) # 映射回 dim 方便与特征相加
        )
        ...
        self.final_proj = nn.Conv1d(dim, channels, 1)
    ...
    def forward(self, x, time, x_self_cond = None, self_cond = None):
        ...
        for norm1, attn, norm2, ff in self.layers:
            # Attention (注意：Attention 类在原 ddpm1d.py 中是 Conv1d 实现，需转回 [B, C, L])
            x_res = rearrange(x, 'b n c -> b c n')
            x_res = norm1(x_res)
            x_res = attn(x_res)
            x = x + rearrange(x_res, 'b c n -> b n c')

            # Feed Forward
            x_res = rearrange(x, 'b n c -> b c n')
            x_res = norm2(x_res)
            x_res = rearrange(x_res, 'b c n -> b n c')
            x = x + ff(x_res)
...
class GaussianDiffusion1D(Module):
    ...
    def p_losses(self, x_start, t, noise = None, model_forward_kwargs: dict = dict(), return_reduced_loss = True, **kwargs):
